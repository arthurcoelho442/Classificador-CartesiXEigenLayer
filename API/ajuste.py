...
def main():
    ...
    for L in valores_L:
        ...
        for i in range(0, len(dados_completos), qtd_Dados):
            dados = dados_completos.iloc[i:i+qtd_Dados, 0:2]  # Pegando as colunas 1 e 2
            if dados.empty:
                break
            
            dados_processados = pd.DataFrame(dados.iloc[:, 0].values.reshape(-1, 1666)) * 10000
            
            # Converter para lista
            dados_json = {"dados": dados_processados.values.tolist()}
